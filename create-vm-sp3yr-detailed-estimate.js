...
async function main() {
  ...
  const builder = new EstimateBuilder('VM Standalone Savings Plan 3yr v1', 'aws');
  ...
  for (const vm of data.ec2_entries) {
    ...
    builder.addService(`ec2Enhancement:${keySafeId}`, {
      ...
      pricingStrategy: { model: 'computeSavings', term: '3yr', upfrontPayment: 'None' },
      storageType: vm.aws_storage_type || 'Storage Cold HDD sc1 GB Mo',
      storageAmount: { value: String(vm.azure_disk_gb || 64), unit: 'gb|NA' },
      snapshotFrequency: '0',
      description: `${vm.machine_id} | ${vm.environment} | ${vm.resource_group} | SP3yr`,
    }, { group });
  ...
  const out = {
    ...
    vmCount: count,
    region: data.region || 'us-east-1',
    pricing: 'Savings Plan 3yr No Upfront',
  };

  fs.writeFileSync(path.join(__dirname, 'vm-sp3yr-detailed-result.json'), JSON.stringify(out, null, 2));
